...
int test_basic_init(void) {
    ...
    TEST_ASSERT(ring_buff_init(&rb, buf, 100) == true, "init should succeed");
    ...
    TEST_ASSERT(ring_buff_available(&rb) == 100, "available should be 100");
    TEST_ASSERT(ring_buff_capacity(&rb) == 100, "capacity should be 100");
...
int test_wrap_around(void) {
    ...
    // remove 4 items (tail move to 4)
    for (int i = 0; i < 4; i++) {
        uint8_t b;
        ring_buff_get(&rb, &b);
    }
    
    // add 4 more items (head gets wrapped around)
    for (int i = 0; i < 4; i++) {
...
int test_peek_and_skip(void) {
    ...
    ring_buff_init(&rb, buf, 10);
    ring_buff_write(&rb, data, 5);
    
    // peek without removing
    size_t peeked = ring_buff_peek(&rb, peek_buf, 3);
    TEST_ASSERT(peeked == 3, "should peek 3 bytes");
    TEST_ASSERT(peek_buf[0] == 1 && peek_buf[1] == 2 && peek_buf[2] == 3, "peek data correct");
    TEST_ASSERT(ring_buff_count(&rb) == 5, "count unchanged after peek");
    
    // skip some bytes
    size_t skipped = ring_buff_skip(&rb, 2);
    TEST_ASSERT(skipped == 2, "should skip 2 bytes");
    TEST_ASSERT(ring_buff_count(&rb) == 3, "count should decrease");
    
    // read remaining
    uint8_t read_buf[10];
    size_t read = ring_buff_read(&rb, read_buf, sizeof(read_buf));
    TEST_ASSERT(read == 3, "should read 3 bytes");
    TEST_ASSERT(read_buf[0] == 3 && read_buf[1] == 4 && read_buf[2] == 5, "remaining data correct");
...
int test_edge_cases(void) {
    ...
    // test NULL pointers
    TEST_ASSERT(ring_buff_init(NULL, buf, 5) == false, "init with NULL rb should fail");
    TEST_ASSERT(ring_buff_init(&rb, NULL, 5) == false, "init with NULL buffer should fail");
    TEST_ASSERT(ring_buff_init(&rb, buf, 0) == false, "init with size 0 should fail");
    
    ring_buff_init(&rb, buf, 5);
    ...
    TEST_ASSERT(ring_buff_skip(&rb, 10) == 0, "skip on empty should return 0");
    
    // test write larger than buffer
    uint8_t large_data[] = "this is way more than 5 bytes";
    size_t written = ring_buff_write(&rb, large_data, sizeof(large_data));
    TEST_ASSERT(written == 5, "should only write 5 bytes");
    TEST_ASSERT(ring_buff_is_full(&rb) == true, "should be full");
...
int test_partial_operations(void) {
    ...
    uint8_t buf[16];  // Make buffer bigger
    ring_buff_t rb;
    ...
    // Write 11 bytes (not 5)
    ring_buff_write(&rb, write_data, 12);
    
    // try to read more than available (request 16, only 11 available)
    size_t read = ring_buff_read(&rb, read_data, 16);
